...
def neuron(input,weight,bias):
    net=input[0]*weight[0]+input[1]*weight[1]+bias

    return sigmoid(net)
def evaluate_fitness(population, input_data, target_output):
    fitness_scores = []
    for individual in population:
        weights = individual[:2]  # Neironning kirish vazirliklari
        bias=individual[2]
        predictions = np.array([neuron(x,weights,bias) for x in input_data])
        #print('p: ',predictions)
        #print('t: ',target_output)
